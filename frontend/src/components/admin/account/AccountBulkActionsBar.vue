...
// NOTE: totalResults/selectingAll/allResultsSelected drive the upstream2
// "select all filtered results" feature, which is only half-wired in
// upstream2's HEAD (AccountsView does not pass them). Made optional so the
// build passes; the select-all UI degrades gracefully (hidden) until wired.
withDefaults(
  defineProps<{
    selectedIds: number[]
    totalResults?: number
